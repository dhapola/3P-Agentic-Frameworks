...
from utils.chat_thread import ChatThreadHelper
...
class ChatThreadListResource(Resource):
    ...
    def get(self):
        ...
        try:
            print('ThreadListResource->get called')
            ...
            logger.error(f"Error getting threads: {str(e)}", exc_info=True)
...
class ChatThreadResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logger.error(f"Error saving thread {chat_thread['thread_id']}: {str(e)}", exc_info=True)
            return {'status': 'error', 'message': str(e)}, 500
