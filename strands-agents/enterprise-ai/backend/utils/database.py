...
class DatabaseManager:
    ...
    def __init__(self, pool_size: int = None, max_overflow: int = None, 
        ...
        if not all([self.db_host, self.db_name, self.db_username, self.db_password]):
            logger.warning("Missing required database configuration. Database functionality will be disabled.")
            logger.warning(f"Required: DB_HOST, DB_NAME, DB_USERNAME, DB_PASSWORD")
            logger.warning(f"Current: host={self.db_host}, name={self.db_name}, username={bool(self.db_username)}, password={bool(self.db_password)}")
            self.engine = None
            self.Session = None
        ...
        try:
            ...
            logger.error(f"Connection details: {self.db_host}:{self.db_port}/{self.db_name} as {self.db_username}")
            self.engine = None
            self.Session = None
    ...
    def execute_query(self, query: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Query executed in {execution_time:.4f} seconds, returned {len(rows)} rows")
            return rows
            
        except Exception as e:
            execution_time = time.time() - start_time
            logger.error(f"Query failed after {execution_time:.4f} seconds: {str(e)}")
    ...
    def execute_write(self, query: str, params: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            logger.debug(f"Write operation executed in {execution_time:.4f} seconds, {rowcount} rows affected")
            return rowcount
            
        except Exception as e:
            execution_time = time.time() - start_time
            logger.error(f"Write operation failed after {execution_time:.4f} seconds: {str(e)}")
            logger.error(f"Query: {query}")
            logger.error(f"Params: {params}")
            raise
    ...
    def execute_batch(self, query: str, params_list: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            logger.debug(f"Batch operation executed in {execution_time:.4f} seconds, {total_rows} total rows affected")
            return total_rows
            
        except Exception as e:
            execution_time = time.time() - start_time
            logger.error(f"Batch operation failed after {execution_time:.4f} seconds: {str(e)}")
            logger.error(f"Query: {query}")
            raise
    ...
    def execute_transaction(self, queries: List[Tuple[str, Optional[Dict[str, Any]]]]) -> bool:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            logger.debug(f"Transaction executed in {execution_time:.4f} seconds")
            return True
            
        except Exception as e:
            execution_time = time.time() - start_time
            logger.error(f"Transaction failed after {execution_time:.4f} seconds: {str(e)}")
            for i, (query, _) in enumerate(queries):
                logger.error(f"Query {i+1}: {query}")
            raise
